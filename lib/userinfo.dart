  ...
  Future<Null> uploadPicture(FirebaseUser user, String picType) async {
    try {
      ...
      final StorageReference ref =
          FirebaseStorage.instance.ref().child('${user.email}_{$picType}');
      ...
      if (picType == 'profilePicture') {
        Firestore.instance
            .collection('users')
            .document(user.uid)
            .updateData({"pic": docUrl});
      } else if (picType == 'frontid') {
        Firestore.instance
            .collection('users')
            .document(user.uid)
            .updateData({"frontid": docUrl});
      } else if (picType == 'backid') {
        Firestore.instance
            .collection('users')
            .document(user.uid)
            .updateData({"backid": docUrl});
      } else if (picType == 'frontlic') {
        Firestore.instance
            .collection('users')
            .document(user.uid)
            .updateData({"frontlic": docUrl});
      }
      setState(() {
        load0 = false;
        load1 = false;
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<QuerySnapshot>(
      stream: Firestore.instance
          .collection('users')
          .where('uid', isEqualTo: data['uid'].toString())
          .snapshots(),
